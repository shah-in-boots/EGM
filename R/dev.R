...
draw_boundary_mask <- function(object) {
  ...
  # Initial validation
  stopifnot(
    "`draw_boundary_mask()` requires a `ggm` object" = inherits(object, "ggm")
  )
  ...
  if (type == 'ecgpuwave') {
    ...
    bounds <-
      ann[, type := ifelse(type == '(', 'onset', type)][,
        ...
        type %in% c('onset', 'offset'),
        .(sample, type, wave)
      ][, beat := seq_len(.N), by = .(wave, type)][,
        beat := paste0(wave, beat)
      ] |> # Beats by group
    ...
    data[wave == 'background', bounds := NA_character_, ][
      wave == 'p',
      bounds := 'darkgoldenrod1'
    ][wave == 'qrs', bounds := 'skyblue4'][wave == 't', bounds := 'indianred3']
  }
  ...
  if (type %in% tolower(.leads$ECG)) {
    ...
    bounds <-
      ...
      ][type == 'onset', wave := start][type == 'offset', wave := end][,
...
add_intervals <- function(
  ...
  # Initial validation
  stopifnot(
    "`add_intervals()` requires a `ggm` object" = inherits(object, "ggm")
  )
  ...
  stopifnot(
    "Intervals not available based on number of beats." = all(
      intervals %in% seq_along(ints)
    )
  )
