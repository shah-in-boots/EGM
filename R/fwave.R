...
process_single_lead <- function(object, lead, qrs_method, f_characteristics) {
  ...
  atrial_activity <- remove_ventricular_signal(
    upsampled_signal,
    method = qrs_method
  )
...
remove_qrs_with_adaptive_svd <- function(
  ...
  if (length(qrs_loc) < 2) {
    warning(
      "Insufficient QRS complexes detected for SVD. Returning original signal."
    )
    return(signal)
  }
  ...
  process_beat_group <- function(
    ...
    svd_result <- tryCatch(
      ...
      error = function(x) {
        warning(
          "SVD failed: ",
          x$message,
          ". Using original signal for this segment group."
        )
        return(NULL)
      }
    )
    ...
    for (i in seq_along(segments)) {
      ...
      if (segment_length <= ncol(template_matrix)) {
        ...
        template <- c(
          template_matrix[i, ],
          rep(0, segment_length - ncol(template_matrix))
        )
      }
      ...
      if (smoothing) {
        ...
        post_region <- (end_idx + 1):min(
          length(signal),
          end_idx + transition_length
        )
      ...
      # Subtract template
      result_signal[start_idx:end_idx] <- result_signal[start_idx:end_idx] -
        template
      ...
      if (smoothing) {
        ...
        if (pre_start < pre_end) {
          ...
          result_signal[pre_region] <- (1 - pre_weights) *
            pre_values +
  ...
  if (aberrant_beats) {
    ...
    if (length(abnormal_indices) > 1) {
      ...
      atrial_signal <- process_beat_group(
        atrial_signal,
        qrs_loc[abnormal_indices],
        abnormal_half_window,
        frequency,
        smoothing
      )
      ...
      atrial_signal[start_idx:end_idx] <- approx(
        ...
        c(
          atrial_signal[max(1, start_idx - 1)],
          atrial_signal[min(length(signal), end_idx + 1)]
        ),
        start_idx:end_idx
      )$y
    }
...
ica_removal <- function(
  ...
  if (length(qrs_loc) < 3) {
    warning(
      "Too few QRS complexes detected for ICA. Returning original signal."
    )
    return(signal)
  }
  ...
  valid_indices <- valid_indices[
    valid_indices > 0 & valid_indices <= length(qrs_sparse)
  ]
  ...
  if (start_idx > 1) {
    ...
    for (i in fade_start:(start_idx - 1)) {
      ...
      atrial_full[i] <- (1 - fade_weights[weight_idx]) *
        signal[i] +
  ...
  if (end_idx < signal_length) {
    atrial_full[(end_idx + 1):signal_length] <- signal[
      (end_idx + 1):signal_length
    ]
  }
...
remove_qrs_with_ica <- function(
  ...
  if (is.null(ica_result)) {
    warning(
      "ICA decomposition failed with all methods. Falling back to adaptive SVD method."
    )
    return(remove_qrs_with_adaptive_svd(signal, frequency = frequency))
  }
...
ica_removal <- function(
  ...
  if (!requireNamespace("fastICA", quietly = TRUE)) {
    warning(
      "Package 'fastICA' not available. Falling back to adaptive SVD method."
    )
    return(remove_qrs_with_adaptive_svd(signal, frequency = frequency))
  }
  ...
  valid_peaks <- qrs_loc[qrs_loc > window_size & qrs_loc <= N - window_size] -
    shift
  ...
  qrs_components <- which(
    qrs_energy / pmax(total_energy, .Machine$double.eps) > threshold
  )
  ...
  if (len_diff > 0) {
    padded_signal <- c(
      padded_signal,
      rep(padded_signal[length(padded_signal)], len_diff)
    )
  }
...
analyze_atrial_signal <- function(
  ...
  if ("approximate_entropy" %in% characteristics) {
    results$approximate_entropy <- analyze_approximate_entropy(
      atrial_signal,
      ...
    )
  }
  if ("dominant_frequency" %in% characteristics) {
    results$dominant_rate <- analyze_dominant_frequency(
      atrial_signal,
      frequency,
      ...
    )
...
detect_QRS <- function(signal, frequency, window_size = 0.150) {
  ...
  is_peak <- (integrated_signal > threshold) &
    (c(
      FALSE,
      integrated_signal[-length(integrated_signal)] < integrated_signal[-1]
    )) &
    (c(
      integrated_signal[-1] > integrated_signal[-length(integrated_signal)],
      FALSE
    ))
...
#'
#' @export
calculate_approximate_entropy <- function(
  x,
  m = 3,
  r = NULL,
  implementation = "C++"
