...
#' @export
header_table <- function(record_name = "", # Record line information
												 ...
												 file_name = "", # Signal specific information
												 storage_format = 16L,
												 ADC_gain = 200L,
	...
	if (length(ADC_saturation) > 0) {
		ADC_gain <- ADC_saturation / digital_gain
	}

	# Signal specifications
	x <- df_list(
		file_name = ifelse(length(file_name) == 0, NA_character_, file_name),
		storage_format = ifelse(length(storage_format) == 0, 16, storage_format),
