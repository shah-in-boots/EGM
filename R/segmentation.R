...
segment_by_sinus <- function(object) {
  ...
  stopifnot(
    'Currently only supports `ecgpuwave` as the annotator' = type == 'ecgpuwave'
  )
  ...
  for (i in beats) {
    ...
    # New beat!
    beatList[[i]] <-
      egm(signal = beatSignal, header = beatHeader, annotation = beatAnnotation)
  }
...
pad_sequence <- function(object, pad, pad_length) {
  ...
  stopifnot(
    "The `pad` argument is not valid" = pad %in% c("before", "after", "both")
  )
  ...
  if (delta < 0) {
    warning(
      "Truncation will occur as there are ",
      maxLength,
      " samples with `pad` set to ",
      pad_length,
      "."
    )
    ...
    before <- stats::setNames(
      data.table(matrix(nrow = left, ncol = length(nm))),
      nm
    )
    before[is.na(before), ] <- 0
    before$sample <- (min(sig$sample) - left):(min(sig$sample) - 1)

    after <- stats::setNames(
      data.table(matrix(nrow = right, ncol = length(nm))),
      nm
    )
    after[is.na(after), ] <- 0
    ...
    if (pad %in% c("before", "both")) {
      before[
        left,
        names(before[, -1]) := ceiling(
          sig[sample == min(sample), .SD, .SDcols = nm[-1]] / 2
        )
      ]
      ...
      after[
        1,
        names(after[, -1]) := floor(
          sig[sample == max(sample), .SD, .SDcols = nm[-1]] / 2
        )
      ]
    }
...
center_sequence <- function(object, center, pad_length) {
  ...
  stopifnot(
    "The `center` variable was not available in the annotation set." = (toupper(
      center
    ) %in%
      toupper(waveTypes))
  )
  ...
  stopifnot(
    "Centering fails if the waveform of interest is not unique in sample." = nrow(ann[
      type == toupper(centerWave) | type == tolower(centerWave)
    ]) ==
      1
  )
  ...
  centerAnn <- copy(ann)
  centerAnn[
    sample >= min(centerSignal$sample) & sample <= max(centerSignal$sample),
  ]
