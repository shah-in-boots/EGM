...
The returned signal carries which of the two it holds, readable with
`signal_units()`. This matters because the difference is a factor of the gain —
200 in the record below — and nothing in the values themselves distinguishes
them: a signal of digital counts read as though it were millivolts is a 200-fold
error that no later check can catch on its own. The label is preserved through
windowing, padding, medians and resampling, and `write_wfdb()` refuses to write a
signal whose label contradicts the units it was told to expect.
...
# Each signal says which units it holds
signal_units(ecg_digital)
#> [1] "digital"
signal_units(ecg_physical)
#> [1] "physical"
...
#> 1  muse-sinus.dat      16     1000            0        mV             16
#> 2  muse-sinus.dat      16     1000            0        mV             16
#> 3  muse-sinus.dat      16     1000            0        mV             16
#> 4  muse-sinus.dat      16     1000            0        mV             16
#> 5  muse-sinus.dat      16     1000            0        mV             16
#> 6  muse-sinus.dat      16     1000            0        mV             16
```

Notice the `ADC_gain` and `ADC_baseline` columns in the header.
In this example, all channels have a gain of 1000 ADC units/mV and a baseline of 0, because MUSE exports its samples in microvolts and there are a thousand of those to the millivolt.
The gain is a property of the recording rather than a constant: a PhysioNet record often carries 200, and many real-world recordings also have non-zero baseline values to account for DC offsets in the recording equipment.
...
# Can add this now to an `egm` object
annotated_ecg <- add_annotation(ecg, ann)

# Output
annotated_ecg
```

The table we built named its channel `"I"`; the one stored on the object holds `1`, the signal number that lead has in this record's header.
`add_annotation()` is where a lead name becomes a number, so that every function reading the annotations matches on the same thing (see `?channels`).
